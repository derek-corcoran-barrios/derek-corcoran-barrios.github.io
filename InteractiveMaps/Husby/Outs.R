...
Husby_Outline <- read_sf("GroupsHusby.shp") %>% 
  st_transform(crs = "+proj=longlat +datum=WGS84 +no_defs") %>% 
  st_union() %>% 
  st_as_sf()
...
saveRDS(Final_ones, "Final_ones_out.husby.rds")
...
writeOGR(ToGPX["name"], driver="GPX", layer="waypoints", 
         dsn="TreatmentsHusby.gpx")

read_sf("TreatmentsHusby.gpx")
