...
\hypertarget{una-variable-categorica-una-continua-1}{%
\subsection{Una variable categórica una
continua}\label{una-variable-categorica-una-continua-1}}
...
Para esto tenemos que poner como x e y en ggplot variables continuas y
agragar la función \texttt{geom\_point} como vemos en el siguiente
...
Si quisieramos que el color de cada punto estuviera separado por peso,
podríamos agregarle \texttt{aes(color\ =\ Diet)} a geom\_point, pero
deberíamos transformar Diet en factor, ya sea antes de usar ggplot o
dentro de ggplot tal como vemos en el siguiente código y en la figura
\ref{fig:scatterColor}.
...
Estas funciones nos permiten generar lineas de tendencias con intervalos
de confianza para estas lineas, así si quisieramos ver lineas de
tendencias para nuestro scatterplot, dependiendo de la dieta, usaríamos
el siguiente código el cual nos da la figura \ref{fig:scatterLoess}.
...
\caption{Gráfico en el cual vemos el peso de pollos en el tiempo, con colores distintos según el tipo de dieta, con lineas de tendencia e intervalos de confianza basados en el método loess}\label{fig:scatterLoess}
\end{figure}

Por defeccto, la función \texttt{geom\_smooth} generará una tendencia
basada en \emph{loess}, lo cual es una correlación local. En general, es
mejor hacer una linea de tendencia basado en modelos que uno puede
...
\caption{Gráfico en el cual vemos el peso de pollos en el tiempo, con colores distintos según el tipo de dieta, con lineas de tendencia e intervalos de confianza basados en modelos lineales}\label{fig:scatterLM}
\end{figure}
...
formula para expresar la relación entre \(x\) e \(y\), por ejemplo, si
...
cuadrática (ver ecuacación \eqref{eq:quad}) generaríamos el siguiente
código que genera la figura \ref{fig:scatterQuad}.
...
\caption{Gráfico en el cual vemos el peso de pollos en el tiempo, con colores distintos según el tipo de dieta, con lineas de tendencia e intervalos de confianza basados en modelos lineales}\label{fig:scatterQuad}
\end{figure}
