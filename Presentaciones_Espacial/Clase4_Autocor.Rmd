...
Spat_pred <- krige(log(zinc)~1, Meuse, Meuse_Grid, model = Abn_fit_Spat) %>% mutate(Modelo = "Espacial")

Dist_pred <- krige(log(zinc)~1, Meuse, Meuse_Grid, model = Abn_fit_Dist) %>% mutate(Modelo = "distancia")

Dist_sq_pred <- krige(log(zinc)~1, Meuse, Meuse_Grid, model = Abn_fit_Dist_sq)  %>% mutate(Modelo = "sqrt(dist)")
...
Spat_CV <- krige.cv(log(zinc)~1, Meuse, model = Abn_fit_Spat, nfold = 5) %>% st_as_sf() %>% mutate(Modelo = "Espacial")

Dist_CV <- krige.cv(log(zinc)~1, Meuse, model = Abn_fit_Dist, nfold = 5) %>% st_as_sf() %>% mutate(Modelo = "distancia")

Dist_sq_CV <- krige.cv(log(zinc)~1, Meuse, model = Abn_fit_Dist_sq, nfold = 5) %>% st_as_sf() %>% mutate(Modelo = "sqrt(dist)")
...
```{r, echo = FALSE}
ggplot(Spat_CV, aes(y = var1.pred, x = observed)) + geom_smooth(method = "lm")+ geom_point() + theme_bw()
```
